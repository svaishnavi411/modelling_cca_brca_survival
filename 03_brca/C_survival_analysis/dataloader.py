...
#         return
    ...
    def feature_generator(self, mode_flag, prior_flag, deflation_flag):
        ...
        mode_list = ['genomics', 'imaging', 'early-fusion', 
                     'pca_genomics', 'pca_imaging', 'pca_concat',
                     'cca', 'scca', 'gnscca', 'gcca']
...
#                      'late-fusion', 'scca', 'gn-scca', 'gcca']
        ...
        if mode_flag == 'pca_genomics':
            features['train'] = self.PCA_genomics_train
            features['valid'] = self.PCA_genomics_valid
            features['test']  = self.PCA_genomics_test
            return features, labels

        if mode_flag == 'pca_imaging':
            features['train'] = self.PCA_imaging_train
            features['valid'] = self.PCA_imaging_valid
            features['test']  = self.PCA_genomics_test
            return features, labels

        if mode_flag == 'pca_concat':
            features['train'] = self.PCA_concat_train
            features['valid'] = self.PCA_concat_valid
            features['test']  = self.PCA_concat_test
            return features, labels
