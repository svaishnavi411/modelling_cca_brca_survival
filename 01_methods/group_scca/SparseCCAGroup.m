function [u, v, cor]=SparseCCAGroup(X, Y, lambda_x, lambda_y, T_x, w_x, T_y, w_y, para)
...
%     end
    ...
    while (iter<maxiter)
        ...
        if (group_x_tag)
            u=group_exgap(X'*(Y*v_old), lambda_x, T_x, w_x, inner_para);
        else
            u=maxL1L2(X'*(Y*v_old), lambda_x);
        end
        u_gap=max(abs(u-u_old));
        u_old=u;

        if (group_y_tag)
            v=group_exgap(Y'*(X*u_old), lambda_y, T_y, w_y, inner_para);                
        else
            v=maxL1L2(Y'*(X*u_old), lambda_y);
        end
...
%          if sum(strcmp(fieldnames(para), 'track')) == 1
%              if para.track == true
%                  U = [U, u_old];
%                  V = [V, v_old];
%              end
...
%      
%          if para.track == true
%              save(strcat('../../results/sim_results/', para.prefix, 'tracking_sparse_cca.mat'), 'U', 'V')
%          end
%      end
end
